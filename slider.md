...
该元素基于 Quasar 的 [QSlider](https://quasar.dev/vue-components/slider) 组件。
...
默认情况下，滑块的值更改事件被节流到 0.05 秒。
这意味着如果您快速移动滑块，该值将仅每 0.05 秒更新一次。

默认情况下，"leading" 和 "trailing" 事件都会被激活。
这意味着第一个事件会立即触发，最后一个事件会在节流时间之后触发。

此演示显示禁用其中任何一个选项如何更改行为。
为了更清楚地看到效果，节流时间设置为 1 秒。
第一个滑块显示默认行为，第二个滑块仅发送前导事件，第三个滑块仅发送尾随事件。
...
| replace: | 要使用的以分号分隔的样式列表，而不是现有的样式列表 |
