...
你可以使用 `validation` 参数来定义一个验证规则字典，例如 `{'Too small!': lambda value: value > 3}`。第一个失败的规则的键将显示为错误消息。或者，你可以传递一个返回可选错误消息的可调用对象。要禁用每次值更改时的自动验证，可以使用 `without_auto_validation` 方法。

| 选项 | 描述 |
| --- | --- |
| label: | 数字输入的显示名称 |
| placeholder: | 如果未输入值时显示的文本 |
| value: | 字段的初始值 |
| min: | 允许的最小值 |
| max: | 允许的最大值 |
...
| validation: | 验证规则字典或返回可选错误消息的可调用对象（默认：None，表示不进行验证） |
...
来自 [Quasar](https://quasar.dev/) 的 `clearable` 属性向输入框添加一个清除文本的按钮。
...
你可以使用 `precision` 参数指定小数位数。负值表示小数点前的位数。当输入失去焦点时，当最小、最大或精度等清理参数更改时，或者当手动调用 `sanitize()` 时，会进行四舍五入。
...
对于异步验证函数，`return_result` 必须设置为 `False`，并且返回值将为 `True`，
与后台评估的验证结果无关。

|  参数 | 描述 |
| --- | --- |
| return_result: | 是否返回验证的结果（默认值：`True`） |
