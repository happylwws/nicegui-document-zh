...
此元素基于 Quasar 的 [QRadio](https://quasar.dev/vue-components/radio) 组件。
...
借助 [`ui.teleport` 元素](teleport)，你可以为单选按钮选项使用任意内容。
...
如果未指定值，则布尔属性默认为 `True`。

|  参数 | 描述 |
| --- | --- |
| add: | 以空格分隔的布尔值或 key=value 对列表，用于添加 |
