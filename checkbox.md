...
该元素基于 Quasar 的 [QCheckbox](https://quasar.dev/vue-components/checkbox) 组件。

| 参数       | 描述                         |
| ---------- | ---------------------------- |
| text:      | 复选框旁显示的标签             |
| value:     | 是否初始选中（默认：`False`） |
| on_change: | 值更改时执行的回调函数         |
...
通过参数传递的 `on_change` 函数将在复选框被点击*和*通过 `set_value` 调用更改值时被调用。要仅在用户与复选框交互时执行函数，可以使用通用的 `on` 方法。
...
标记用于使用 [ElementFilter](/documentation/element_filter) 查询元素，这在测试中大量使用，但也可用于减少全局变量的数量或传递依赖项。
