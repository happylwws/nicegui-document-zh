...
基于 [nipple.js](https://yoannmoi.net/nipplejs/) 创建一个摇杆。

| 参数        | 描述                                       |
| ----------- | ------------------------------------------ |
| on_start:   | 当用户触摸摇杆时的回调函数                     |
| on_move:    | 当用户移动摇杆时的回调函数                     |
| on_end:     | 当用户释放摇杆时的回调函数                     |
| throttle:   | 移动事件的节流间隔（秒），默认为 0.05         |
| options:    | 传递给[底层 nipple.js 库](https://github.com/yoannmoinet/nipplejs#options)的参数，例如 `color` |
...
| args: | 发送到事件处理程序的消息中包含的参数（默认：`None`，表示全部） |
...
**`on_end`**`(callback: Union[Callable[[nicegui.events.JoystickEventArguments], Any], Callable[[], Any]]) -> Self`

添加一个回调，以便在用户释放摇杆时调用。

**`on_move`**`(callback: Union[Callable[[nicegui.events.JoystickEventArguments], Any], Callable[[], Any]]) -> Self`

添加一个回调，以便在用户移动摇杆时调用。

**`on_start`**`(callback: Union[Callable[[nicegui.events.JoystickEventArguments], Any], Callable[[], Any]]) -> Self`
