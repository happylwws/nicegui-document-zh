...
创建一个日志视图，允许添加新行，而无需将整个历史记录重新传输到客户端。

| 参数 | 描述 |
| --- | --- |
| max_lines: | 在删除最旧的行之前的最大行数（默认: `None`） |
...
你可以将一个 `ui.log` 元素附加到一个 Python 的 logger 对象，这样日志消息就会被推送到日志元素中。
...
NiceGUI 使用 Vue 中的插槽概念：
元素可以有多个插槽，每个插槽可能都有多个子元素。
大多数元素只有一个插槽，例如 `ui.card` (QCard) 只有一个默认插槽。
但是，更复杂的元素（如 `ui.table` (QTable)）可以有更多插槽，例如 “header”、“body” 等。
如果使用 `ui.row(): ...` 通过 with 嵌套 NiceGUI 元素，则将新元素放置在行的默认插槽中。
但是，如果使用 with `table.add_slot(...): ...`，则会进入不同的插槽。

插槽堆栈可帮助 NiceGUI 跟踪当前哪个插槽用于新元素。
`parent` 字段保存对其元素的引用。
...
| forward: | 在应用于目标之前应用于该值的函数。 |
| backward: | 在应用于此元素之前应用于该值的函数。 |
