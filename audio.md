...
| controls   | 是否显示音频控制，例如播放、暂停和音量 (默认: `True`)                         |
| autoplay   | 是否自动开始播放音频 (默认: `False`)                                      |
| muted      | 音频是否应初始静音 (默认: `False`)                                       |
| loop       | 音频是否应循环播放 (默认: `False`)                                     |

有关您可以使用通用事件订阅 `on()` 订阅的事件列表，请参阅[此处](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio#events)。
...
此演示展示了如何订阅一些[可用事件](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/audio#events)。
...
标记用于通过 [ElementFilter](documentation/element_filter) 识别元素，该元素在测试中大量使用，但也可以用于减少全局变量的数量或传递依赖项。
