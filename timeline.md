...
这个元素代表 [Quasar 的 QTimeline](https://quasar.dev/vue-components/timeline#qtimeline-api) 组件。

|  选项 | 描述 |
| --- | --- |
| side: |  边（"left" 或 "right"；默认值: "left"）。 |
| layout: |  布局（"dense"、"comfortable" 或 "loose"；默认值: "dense"）。 |
| color: |  图标的颜色。 |
...
| replace: | 以空格分隔的要使用的类字符串，而不是现有的类字符串 |
...
| replace: | 以分号分隔的要使用的样式列表，而不是现有的样式列表 |
