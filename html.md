...
将任意 HTML 渲染到页面上，并包裹在指定的标签中。
可以使用 [Tailwind](https://tailwindcss.com/) 进行样式设置。
您还可以使用 `ui.add_head_html` 将 HTML 代码添加到文档的头部，并使用 `ui.add_body_html` 将其添加到主体中。

| 参数 | 描述 |
| --- | --- |
| content: | 要显示的 HTML 代码 |
| tag: | 用于包裹内容的 HTML 标签（默认: "div"） |
...
它等效于使用带有 `tag` 参数的 `ui.element` 方法。

与其他任何元素一样，您可以添加类、样式、属性、工具提示和事件。
一个便利之处是，关键字参数会自动添加到元素的 `props` 字典中。
