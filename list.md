...
一个基于 Quasar 的 [QList](https://quasar.dev/vue-components/list-and-list-items#qlist-api) 组件的列表元素。它为 `ui.item` 元素提供了一个容器。
...
列表项使用 item sections 来组织它们的内容。Item labels 根据其 props 占据不同的位置。
...
这允许使用 [Quasar](https://quasar.dev/) props 来修改元素的外观或其布局。
...
| --- | --- |
| text: | 要显示的文本（默认值：""） |
| on_click: |  单击项目时要执行的回调（将“clickable” prop 设置为 True） |
