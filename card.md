...
注意：
与此元素相比，原始 QCard 默认没有内边距，并且会隐藏嵌套元素的外边框和阴影。
如果想要原始行为，请使用 `tight` 方法。

| 选项 | 描述 |
| --- | --- |
| align_items: | 卡片中项目的对齐方式（"start"、"end"、"center"、"baseline" 或 "stretch"；默认值：`None`） |
...
你可以通过添加 `no-shadow` 类来移除卡片的阴影。
以下演示显示了一个 1 像素宽的边框。

或者，你可以使用 Quasar 的 "flat" 和 "bordered" 属性来实现相同的效果。
...
默认情况下，卡片具有内边距。
你可以使用 `tight` 方法删除内边距和嵌套元素之间的间隙。
这还会隐藏嵌套元素的外边框和阴影，就像在原始 QCard 中一样。
