...
设置 [Quasar](https://quasar.dev/style/theme-builder) 使用的主颜色（primary, secondary, accent, ...）。

|  选项         | 描述                         |
| ----------- | ---------------------------- |
| primary:    | 主颜色 (默认: "#5898d4")         |
| secondary:  | 次要颜色 (默认: "#26a69a")       |
| accent:     | 强调颜色 (默认: "#9c27b0")       |
| dark:       | 深色 (默认: "#1d1d1d")          |
| dark_page:  | 深色页面颜色 (默认: "#121212")    |
| positive:   | 肯定色 (默认: "#21ba45")        |
| negative:   | 否定色 (默认: "#c10015")        |
| info:       | 信息色 (默认: "#31ccec")        |
| warning:    | 警告色 (默认: "#f2c037")        |
| custom_colors: | 用于品牌定制的颜色定义 (需要在首次使用自定义颜色之前调用 `ui.colors`) |
...
你可以添加自定义的品牌颜色定义。
在这种情况下，必须在首次使用自定义颜色之前调用 `ui.colors`。
...
标记用于使用 [ElementFilter](/documentation/element_filter) 标识元素以进行查询，这在测试中大量使用，但也可用于减少全局变量的数量或传递依赖项。
